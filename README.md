...
## Version 2.1.1
- 発話不能になっていた問題を修正します。
  具体的には、wedata様 が使えなくなったことによりServiceWalkerが起動できていなかった問題に対応しています。

## Version 2.1.2
- 「停止」や「中断」を押しても読み上げが止まらないことがある問題を修正しました。
- 設定ページでのテスト再生(試聴)の声と、実際の読み上げの声が食い違うことがある問題を修正しました。
- 新しく開いたタブでも、キーボードショートカット等から読み上げを開始しやすくなりました。
- 使わなくなった権限(tts)を削除し、過去の名残で複雑になっていた発話まわりの内部コードを整理しました。
- iOS やタブレットへの対応コードを入れ始めました
- 画面内に発話開始・停止用のフローティングボタンを配置できるようにします。タッチ操作の端末では標準で表示されます。
- 2本指タップで読み上げの開始・停止をできるようにする設定を追加しました。
- 範囲選択できないページでも選択できるように努力する設定を追加しました(CSS で選択を禁止しているページには有効ですが、JavaScript で選択を打ち消しているページなど、効果のない場合もあります)。

## Version 2.1.3
- 「強制スクロールの追従のゆとり」オプションを追加しました。既存の動作が少し変わりますが、このオプションの値を0にすると既存の動作に戻せはします。

## Version 2.1.4
- 本文を Shadow DOM の中に描画するサイト(MSN など最近の Web Components を使ったページ)で、本文がまったく読み上げられない・「続きを読む」など別の場所から読み始めてしまう問題を修正しました。Shadow DOM(open)を貫通して本文を抽出するようにし、その結果として範囲選択や両ボタンクリックで指定した位置からきちんと読み上げられるようになります。
- SiteInfo に本文位置の登録が無いページで「選択せずに」読み上げを開始したとき、ページ先頭(ナビやヘッダー)からではなく、本文らしいブロックを推定してそこから読み始めるようにしました(ニュース記事・ブログ・複数記事が並ぶフィードで有効。複数記事のときは今表示している記事を優先します)。見出しごとに細かく分割された巨大な記事(Wikipedia 等)では導入部を飛ばすことがあります。確実に読みたい場所は範囲選択してから開始してください。
